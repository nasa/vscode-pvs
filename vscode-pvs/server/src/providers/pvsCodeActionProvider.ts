...
import { PvsTheory, quickFixReplaceCommand, QuickFixReplace, QuickFixAddImporting, quickFixAddImportingCommand, PvsContextDescriptor, LookUpTable, FileDescriptor, PvsFileDescriptor, FileList, PvsFile, PvsTypeDecl, PvsFormula } from "../common/serverInterface";
import { nasalib_lookup_table } from "../common/nasalib-lookup-table";
import * as fsUtils from '../common/fsUtils';
import { errorCannotFindTheoryRegExp, expectingTypeRegExp, tccSourceMessage, theoremParamsRegexp, theoremRegexp, theoryRegexp } from "../common/languageUtils";
...
export class PvsCodeActionProvider extends fsUtils.PostTask {
     ...
     */
    fixProveTcc (file: FileDescriptor, diag: Diagnostic): CodeAction[] {
        if (file?.fileName && diag?.source) {
            const data: string = diag.source.replace(tccSourceMessage, "").trim();
            ...
            const match: RegExpMatchArray = regex.exec(data);
            if (match?.length > 3 && match[1] && match[2] && match[3]) {
                const file: PvsFile = fsUtils.fname2desc(match[1]);
                const theoryName: string = match[2];
                const formulaName: string = match[3];
            ...
            if (context.diagnostics?.length) {
                ...
                for (let i = 0; i < context.diagnostics.length; i++) {
                    ...
                    if (diag.message) {
                        ...
                        const proveTcc: RegExp = new RegExp(tccSourceMessage, "gi");
                        if (proveTcc.test(diag.source)) {
                            const fixes: CodeAction[] = this.fixProveTcc(fdesc, diag);
                            actions = actions.concat(fixes);
                        }
                    }
                }
