...
import { Pvs2Latex, Pvs2LatexSettings } from "./pvs2latex";
...
export interface PvsDocCliOptions extends Pvs2LatexSettings { };
...
async function generateDocFiles () {
    const worker: Pvs2Latex = new Pvs2Latex();
    console.log(process.argv)
    const options: PvsDocCliOptions = parseCliArgs(process.argv.slice(2));
    if (options?.inputFolder) {
        const desc: PvsDocDescriptor | null = await worker.run(options); // the first two args are 'node' and 'pvs2latex.js'
        if (desc?.mainFile) {
            console.log("------------------------");
