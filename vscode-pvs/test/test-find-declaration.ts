...
describe("pvs-proxy", () => {
    ...
    it(`can invoke find-declaration`, async () => {
        ...
        const response: PvsResponse | undefined = await pvsProxy?.findDeclaration("sqrt");
        ...
        if(response != undefined){
            ...
            expect(test.find_declaration_result[0].filename).not.to.be.undefined;
            if(test.find_declaration_result[0].filename != undefined)
    ...
    it(`is robust to multiple parallel invocation of find-declaration`, async () => {
        ...
        const response: PvsResponse | undefined = await pvsProxy?.findDeclaration("forall_or");
