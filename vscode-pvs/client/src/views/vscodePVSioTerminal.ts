...
export class PVSioTerminal {
    pvsioExecutable: string;
    pvsPath: string;
    terminal: vscode.Terminal;
    constructor (desc: { fileName: string, fileExtension: string, contextFolder: string, theoryName: string }) {
        const tname: string = `PVSio ${desc.theoryName}`;
        this.pvsPath = getPvsPath();
        this.pvsioExecutable = path.join(this.pvsPath, "pvsio");
        const fname: string = fsUtils.desc2fname(desc);
        const args: string[] = [ this.pvsioExecutable, fname + "@" + desc.theoryName ];
...
export class VSCodePVSioTerminal {
    ...
    activate (context: vscode.ExtensionContext) {
        // context.subscriptions.push(vscode.commands.registerCommand("vscode-pvs.pvsio-evaluator", () => {
