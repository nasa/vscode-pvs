...
import { TreeItem, Command, TreeItemCollapsibleState, TreeDataProvider, EventEmitter, Event, ExtensionContext } from "vscode";
...
export class VSCodePvsProofMate implements TreeDataProvider<TreeItem> {
    ...
    addRecommendations (rec: { cmd: string, tooltip: string }[]): void {
        if (rec) {
            this.nodes = rec.map(r => {
                return new ProofMateItem(r.cmd, r.tooltip);
            });
        }
    }
