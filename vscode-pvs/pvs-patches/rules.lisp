...
(defun help-rule (rule-name)
  (cond 
	((string= rule-name '*)
	 (help-rule 'rules)
	 (help-rule 'defined-rules)
	 (help-rule 'strategies))
	((string-equal rule-name 'rules)
	 (format-if "~%~70,,,'-@a" "")
	 (format-if "~%The rules (in alphabetical order) are: ~%")
	...
	((string-equal rule-name 'defined-rules)
	 (format-if "~%~70,,,'-@a" "")
	 (format-if "~%The defined rules (in alphabetical order) are: ~%")
	 (loop for x in (sort-hashtable *rules* :key #'name)
	       unless (defhelper-entry? x)
	       do (format-if "~%~a~%" (docstring x))))
	((string-equal rule-name 'strategies)
	 (format-if "~%~70,,,'-@a" "")
	 (format-if "~%The strategies (in alphabetical order) are: ~%")
	...
	(t (let ((entry (prover-command-entry rule-name)))
	     (if (null entry)
		 (format-if "~%No such rule, defined rule or strategy.~%")
		 (format-if "~%~a~%" (docstring entry)))))))
